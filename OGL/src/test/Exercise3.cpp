...
namespace E3
{
	...
	PerspectiveCamera camera1(PerspectiveFrustum(ratio), vec3(0.0f, 0.0f, 3.0f));
	OrthoGraphicCamera camera2(OrthoFrustum(-w/2.0f, w/2.0f, -h/2.0f, h/2.0f), vec3(0.0f, 0.0f, 3.0f));
	...
	void RenderScene(GLFWwindow* window)
	{
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			//  projection matrix
			auto projection = camera1.GetProjectionMatrix();
			shaderProgram.SetUniform("projection", projection.ValuePtr());

			// camera/view transformation
			auto view = camera1.GetViewMatrix();
	...
	void mouse_callback(GLFWwindow* window, double xpos, double ypos)
	{
		...
		camera1.OnMouseMove(xoffset, yoffset);
	}

	void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
	{
		camera1.OnMouseScroll(yoffset);
		camera2.OnMouseScroll(yoffset);
	}
	...
	void processWindowInput(GLFWwindow* window)
	{
		...
		if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
			camera1.Move(Camera_Movement::FORWARD, deltaTime);
		if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
			camera1.Move(Camera_Movement::BACKWARD, deltaTime);
		if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
			camera1.Move(Camera_Movement::LEFT, deltaTime);
		if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
			camera1.Move(Camera_Movement::RIGHT, deltaTime);

		if (glfwGetKey(window, GLFW_KEY_LEFT_ALT) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_RIGHT_ALT) == GLFW_PRESS)
		{
			camera2.SetRotateState(true);
		}
		else if (glfwGetKey(window, GLFW_KEY_LEFT_ALT) == GLFW_RELEASE && glfwGetKey(window, GLFW_KEY_RIGHT_ALT) == GLFW_RELEASE)
		{
			camera2.SetRotateState(false);
		}
	}
}